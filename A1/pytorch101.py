...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # reach_stop = False
    # count = -1
    # while not reach_stop:
    #     count += 1
    #     if 10 * count >= start and 10 * count <= stop:
    #         tens.append(10 * count)
    #     elif 10 * count > stop:
    #         reach_stop = True
    tens = [10 * count for count in range((start+9) // 10, (stop // 10) + 1)]
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x[[i for i in range(x.shape[0]-1, -1, -1)]]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    Returns:
        y: Tensor of shape (N, C) and where C = 1 + max(x) is one more than the
            ...
            of x[n]; in other words, if x[n] = c then y[n, c] = 1; all others
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.nn.functional.one_hot(torch.tensor(x), num_classes=-1).to(torch.float32)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    col_to_change = y.argmin(dim=1, keepdim=True).reshape(-1)
    y[torch.arange(y.shape[0]), col_to_change] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    y = x.clone()
    y = y.t()
    column_mean = torch.sum(y, dim=1) / (y.shape[1])
    column_std = torch.sqrt((torch.sum(((y - column_mean.view(-1, 1)) ** 2), dim=1)) / (y.shape[1] - 1))
    y = (y - column_mean.view(-1, 1)) / column_std.view(-1, 1)
    y = y.t().to(x)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
